...
"""Benchmarks channel write using packed message implementation"""
...
class SpikeInjectionPackedSuite(PerfLoggingHandler):
    """Benchmarks channel write using packed message implementation"""
    ...
    def setup(self, tsteps=100, numAxons=10):
        ...
        cPath = os.path.dirname(os.path.realpath(__file__))+"/spiking_packed.c"
        ...
        self.spkChannel = self.board.createChannel(
            b'nxspk', "packed", tsteps * 4 * numAxons * 4)
        ...
        data = []
        cnt = 0
        if ((tsteps * numAxons) - cnt) >= 3:
            data.append(3)
        else:
            data.append(tsteps * numAxons)
            ...
            for axonid in range(numAxons):
                ...
                if (cnt % 3 == 0):
                    if ((tsteps * numAxons) - cnt) >= 3:
                        ...
                        data.extend([0, 0, 0])
                        if (tsteps * numAxons) - cnt == 0:
                            data.append(-1)
                        else:
                            data.append((tsteps * numAxons) - cnt)
                elif (tsteps * numAxons) - cnt == 0:
                    data.append(-1)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Spike Injection Packed Test Suite')
    parser.add_argument('tsteps', type=int, nargs="?",
                        default=100, help='Number of time steps')
    parser.add_argument('numAxons', type=int, nargs="?",
                        default=10, help='Number of Axons')

    args = parser.parse_args()
    suite = SpikeInjectionPackedSuite()
    suite.setup(tsteps=args.tsteps, numAxons=args.numAxons)
