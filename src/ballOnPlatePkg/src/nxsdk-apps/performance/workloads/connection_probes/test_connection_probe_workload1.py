...
"""Workload to benchmark a network of 200 compartments, 1 connection, weight probe"""
...
class WorkloadConnectionProbe1Suite(PerfLoggingHandler):
    """Workload to benchmark a network of 200 compartments, 1 connection, weight probe"""
    @timeit
    @memit
    def time_workload(self):
        ...
        conn = cg1[0].connect(cg2[0], prototype=connProto1)
        conn.probe([nx.ProbeParameter.SYNAPSE_WEIGHT], None)
        net.run(100)
        net.disconnect()


if __name__ == '__main__':
    WorkloadConnectionProbe1Suite().time_workload()
