...
void ClearSomaState(){
    for(int i=0;i<numCx;i++){
        SomaState temp = neuron->soma_state[i];
        temp.A= 0;
        neuron->soma_state[i] = temp;
    }
}
...
void run_mgmt(runState *RunState) {
    ...
    if(trainingPhase == 1) {
        ...
        // b) Set the learning rules corresponding to it
            ...
            //int index = (RunState->time_step/timestepPerImage)%numTrainImages;
            readChannel(labelChannelId, &imageLabels, 1);
            SetLabel(imageLabels);
        }
    }
    // For inference clear soma states after inferring each image
    else{
       ClearSomaState();
    }
