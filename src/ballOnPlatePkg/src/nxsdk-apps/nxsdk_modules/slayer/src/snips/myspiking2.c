...
int do_spiking2(runState *s) {
    return 1;
}

void run_spiking2(runState *s) {

    time = s->time_step;
    
    // initialize
    if (time==1){
        channelID = getChannelID("spikeAddresses2");
        if(channelID == -1) {
              printf("Invalid channelID for spikeAddresses2\n");
        }
