...
#
# Two types of learning rules are illustrated, including E-STDP.  For each,
...
def setupNetwork(learningExample):
    ...
    if learningExample is 'estdpNeg':
        ...
        learningRule = 'dw = x1*y0-2*y1*x0'
        spikeTimes0 = [5, 30, 45]
        spikeTimes1 = [15, 40, 60]
    elif learningExample is 'estdpPos':
        ...
        learningRule = 'dw = 4*x1*y0-y1*x0'
        spikeTimes0 = [15, 40, 60]
        spikeTimes1 = [5, 30, 45]
    elif learningExample is 'otherPos':
        # Since y2 is configured to decay faster than y1, in this example,
        # dw > 0.  The post synaptic trace is triggered by a spike in
        # compartment 0 caused by an input spike into axon 0 at ts 5.  When
        # the pre-synaptic spike occurs at ts 15, the learning rule outputs
        # a weight increase.  As a result, the pre-synaptic spike occurring
        # at ts 40 leads to a higher current value.
        learningRule = 'dw = 3*y1*x0-3*y2*x0'
        spikeTimes0 = [5, 30, 45]
        spikeTimes1 = [15, 40, 60]
    elif learningExample is 'otherNeg':
        # Since y2 is configured to decay faster than y1, this example results
        # in dw < 0 (i.e. the updated learning rule has the negative scale
        # factor associated with the y1*x0 term.  As a result, the current
        # jump due to the pre-synaptic spike at ts 40 is lower than the current
        # increase seen at ts 15.
        learningRule = 'dw = -2*y1*x0+2*y2*x0'
        spikeTimes0 = [5, 30, 45]
        spikeTimes1 = [15, 40, 60]
    else:
