...
# -----------------------------------------------------------------------------
# Tutorial: tutorial_13_axonal_delay.py
# -----------------------------------------------------------------------------
#
# This tutorial introduces axonal delay. We configure two independent neurons
# each connected to a separate compartment through a synapse. We set
# different axonal delays for each neuron to illustrate the delay from
# when the neuron generates the spike to when the spike is actually sent out to
...
def setupNetwork():
    ...
    # are also indices 0, 1, 2, 3 i.e. cxCfg[0], cxCfg[1], cxCfg[2] and cxCfg[3]
    ...
    # Configure axonal delay of 4 for neuron 0
    n2Core.somaState[0].configure(axonDelay=4)

    # Configure axonal delay of 21 for neuron 2
    n2Core.somaState[2].configure(axonDelay=21)
    ...
    # vth*(2^6) = 50*2^6 = 3200
    n2Core.vthProfileCfg[0].staticCfg.configure(vth=50)
...
if __name__ == "__main__":
    ...
    # Compartment 0 receives the input spike at time step 3 at which point u0 increases
    fig = plt.figure(1002)
    plt.subplot(4, 2, 1)
    uProbes[0].plot()
    plt.title('u0')
    ...
    # Compartment 0 has an axonal delay of 4 and is connected to compartment 1.
    # u1 accumulates at time step 9. The spike arrived at compartment 0 at time step 3,
    # is processed on time step 4 and delayed for time steps 5, 6 and 7, sent to compartment 1
    # on time step 8 to be accumulated in u1 at time step 9.
    plt.subplot(4, 2, 3)
    uProbes[1].plot()
    plt.title('u1')
    plt.subplot(4, 2, 4)
    ...
    # Compartment 2 receives the input spike at time step 3 at which point u2 increases
    plt.subplot(4, 2, 5)
    uProbes[2].plot()
    plt.title('u2')
    plt.subplot(4, 2, 6)
    ...
    # Compartment 2 has an axonal delay of 21 and is connected to compartment 3.
    # u3 accumulates at time step 26. The spike arrived at compartment 2 at time step 3,
    # is processed on time step 4 and delayed until time step 25 when it is sent to
    # compartment 3 and accumulated in u3 at time step 26.
    plt.subplot(4, 2, 7)
    uProbes[3].plot()
    plt.title('u3')
    plt.subplot(4, 2, 8)
    ...
    if haveDisplay:
        ...
        fileName = "tutorial_13_fig1002.png"
