...
class controller: 
    def __init__(self) -> None:
        ...
        self.sim, self.board, self.hw = self.setupLoihi(self.model)
        # Init ROS node
    ...
    def build(self): 
        ...
        with model:
            ...
            self.output = nengo.Node(output=self.process_data, size_in=1)
            ...
            nengo.Connection(self.intermediateX, self.output, synapse=0.001)
            #nengo.Connection(self.intermediateY, self.input[1], synapse=0.001)
    ...
    def process_data(self, t, x):
        rospy.loginfo(x)
    
    def setupLoihi(self, model): 
        os.environ.update({'KAPOHOBAY': '1'})
        nengo_loihi.set_defaults()
        sim = nengo_loihi.Simulator(model, dt=self.dt, progress_bar=False, target='loihi', hardware_options={"snip_max_spikes_per_step": 300, "n_chips": 1})
    ...
    def __call__(self, t, values):
        ...
        #lo = sim.data[self.probeX][self.timestep]
        #rospy.loginfo("Servo Data: %s", print(np.shape(lo)))
    ...
    def initNode(self):
        ...
        sub = rospy.Subscriber('touchscreenData', Float32MultiArray, callback=self.callback, queue_size=1)
        pub = rospy.Subscriber('servoData', Float32MultiArray, queue_size=1)
       #pubServo = rospy.Publisher('servoData', Float32MultiArray, queue_size=10)
        return pub, sub
    ...
    def get_sim_hw_board(self): 
        return self.sim, self.hw, self.board
...
if __name__ == '__main__':
    ...
    try:
        ...
        # ax2.scatter(t_values, probes_values_Y, color='red', label='Y-axis')
